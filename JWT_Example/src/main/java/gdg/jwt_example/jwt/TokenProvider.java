...
public class TokenProvider {
    ...
    public String createAccessToken(Driver driver) {
        long nowTime = (new Date().getTime());
        Date accessTokenExpiredTime = new Date(nowTime + accessTokenValidityTime);

        return Jwts.builder()
                .subject(driver.getId().toString()) // 토큰 제목(사용자 이름)
                .claim(ROLE_CLAIM, driver.getRole().name()) // 권한 정보
